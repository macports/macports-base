...
** If it is SQLITE_TEXT or SQLITE_BLOB, then a text (sqlite3_result_text64())
  ...
  if( rc && rc!=TCL_RETURN ){
    ...
    switch( eType ){
      ...
      default: {
        ...
        sqlite3_result_text64(context, (char *)data, n, SQLITE_TRANSIENT, SQLITE_UTF8);
        break;
      }
    }
      ...
      if( pVar ){
        ...
        if( zVar[0]=='@' ||
          ...
          ** parameter name begins with "@". */
          ...
          data = (unsigned char *)Tcl_GetStringFromObj(pVar, &n);
          sqlite3_bind_text64(pStmt, i, (char *)data, n, SQLITE_STATIC, SQLITE_UTF8);
          Tcl_IncrRefCount(pVar);
          pPreStmt->apParm[iParm++] = pVar;
        }
      }else{
    ...
    while( Tcl_Gets(in, &str)>=0 ) {
      ...
      for(i=0; i<nCol; i++){
        ...
        if( (nNull>0 && strcmp(azCol[i], zNull)==0)
          ...
          sqlite3_bind_text64(pStmt, i+1, azCol[i], -1, SQLITE_STATIC, SQLITE_UTF8);
        }
      }
      sqlite3_step(pStmt);
...
DLLEXPORT int Sqlite3_Init(Tcl_Interp *interp){
  ...
  if( rc==TCL_OK ){
    ...
    ** command. */
	...
	Tcl_CreateObjCommand(interp, "::sqlite3::build-info",
		info.objProc, (void *)(
		    PACKAGE_VERSION "+" STRINGIFY(SQLITE3_VERSION_UUID)
...
DLLEXPORT int Sqlite_Unload(Tcl_Interp *interp, int flags){ return TCL_OK; }
DLLEXPORT int Tclsqlite_Unload(Tcl_Interp *interp, int flags){ return TCL_OK; }
DLLEXPORT int Sqlite_SafeInit(Tcl_Interp *interp){ return TCL_ERROR; }
DLLEXPORT int Sqlite_SafeUnload(Tcl_Interp *interp, int flags){return TCL_ERROR;}

/* Also variants with a lowercase "s" */
DLLEXPORT int sqlite3_Init(Tcl_Interp *interp){ return Sqlite3_Init(interp);}
DLLEXPORT int sqlite_Init(Tcl_Interp *interp){ return Sqlite3_Init(interp);}
#ifdef __cplusplus
}
#endif  /* __cplusplus */
