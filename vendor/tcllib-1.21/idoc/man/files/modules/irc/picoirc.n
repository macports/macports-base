...
'\"
.TH "picoirc" n 0\&.13\&.0 tcllib "Simple embeddable IRC interface"
...
.sp
package require \fBpicoirc  ?0\&.13\&.0?\fR
.sp
...
login using the \fInick\fR as the username and optionally
\fIpassword\fR\&.  If the \fIurl\fR starts with \fIircs://\fR then a TLS
connection is created\&. The \fIcallback\fR must be as specified in
\fBCALLBACK\fR\&.  Returns a package-specific variable that is used
...
server\&. If \fImessage\fR is multiline then each line will be processed
...
IRC command sequences and then sent\&. If \fIchannel\fR is empty then
all raw output to the server is handled\&. The default action is to
write the \fImessage\fR to the irc socket\&. However, before this
...
received\&.
.TP
\fBuserinfo\fR \fInick\fR \fIinfo\fR
called as a response of WHOIS command\&. \fInick\fR is the user
the command was targeted for\&. \fIinfo\fR is the dictionary containing
...
message was sent as a CTCP ACTION\&. \fItype\fR is set to "NOTICE" if the
message was sent as a NOTICE command, in that case \fItarget\fR is empty
if it matches current user nick or it's "*", in later case empty
\fItarget\fR means that notice comes from server\&.
.TP
\fBmode\fR \fInick\fR \fItarget\fR \fIflags\fR
called when mode of user or channel changes\&. \fInick\fR is the name
of the user who requested a change, can be empty if it's the server\&.
\fItarget\fR is the identity that has its mode changed\&. \fIflags\fR
