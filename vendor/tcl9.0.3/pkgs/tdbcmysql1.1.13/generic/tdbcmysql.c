...
# define Tcl_NewWideUIntObj(i) Tcl_NewWideIntObj((Tcl_WideInt)(i))
...
unsigned long mysqlClientVersion = 0;
				/* Version number of MySQL */
Tcl_Obj* mysqlClientLibName = NULL;
				/* Name of the MySQL client library */
int mysqlClientAtLeast51;       /* Flag == 1 if MySQL client is at least
				 * version 5.1.0 */
...
static MYSQL_BIND* MysqlBindAlloc(size_t nBindings);
static MYSQL_BIND* MysqlBindIndex(MYSQL_BIND* b, Tcl_Size i);
static void* MysqlBindAllocBuffer(MYSQL_BIND* b, Tcl_Size i, unsigned long len);
static void MysqlBindFreeBuffer(MYSQL_BIND* b, Tcl_Size i);
static void MysqlBindSetBufferType(MYSQL_BIND* b, Tcl_Size i,
				   enum enum_field_types t);
static void* MysqlBindGetBuffer(MYSQL_BIND* b, Tcl_Size i);
static unsigned long MysqlBindGetBufferLength(MYSQL_BIND* b, Tcl_Size i);
static void MysqlBindSetLength(MYSQL_BIND* b, Tcl_Size i, unsigned long* p);
static void MysqlBindSetIsNull(MYSQL_BIND* b, Tcl_Size i, my_bool* p);
static void MysqlBindSetError(MYSQL_BIND* b, Tcl_Size i, my_bool* p);

static MYSQL_FIELD* MysqlFieldIndex(MYSQL_FIELD* fields, Tcl_Size i);
...
static MYSQL_STMT* AllocAndPrepareStatement(Tcl_Interp* interp,
					    StatementData* sdata);
static Tcl_Obj* ResultDescToTcl(MYSQL_RES* resultDesc);
...
const static Tcl_MethodType2 ConnectionEvaldirectMethodType = {
    TCL_OO_METHOD_VERSION_2,
				/* version */
    "evaldirect",		/* name */
...
const static Tcl_MethodType2 ConnectionNeedCollationInfoMethodType = {
    TCL_OO_METHOD_VERSION_2,
				/* version */
...
const static Tcl_MethodType2 ConnectionSetCollationInfoMethodType = {
    TCL_OO_METHOD_VERSION_2,
				/* version */
...
static MYSQL_BIND*
MysqlBindAlloc(size_t nBindings)
{
    size_t size;
    void* retval = NULL;
...
MysqlBindAllocBuffer(
    ...
    Tcl_Size i,			/* Index into the array */
...
QueryConnectionOption (
    ...
    unsigned fieldCount;		/* Number of fields in a row */
	...
	if ((row = mysql_fetch_row(result)) == NULL) {
	    ...
	    retval = Tcl_NewStringObj(row[1], (Tcl_Size)lengths[1]);
	}
    }
    mysql_free_result(result);
    return retval;
...
ConfigureConnection(
	...
	if (objc == skip) {
	    retval = Tcl_NewObj();
	    for (j = 0; ConnOptions[j].name != NULL; ++j) {
		if (ConnOptions[j].flags & CONN_OPT_FLAG_ALIAS) continue;
		optval = QueryConnectionOption(cdata, interp, j);
		if (optval == NULL) {
		    return TCL_ERROR;
		}
		Tcl_DictObjPut(NULL, retval,
			       Tcl_NewStringObj(ConnOptions[j].name, -1),
			       optval);
	    }
	...
	case TYPE_FLAG:
	    ...
	    if (optionValue) {
		mysqlFlags |= (unsigned long)ConnOptions[optionIndex].info;
	    }
	    break;
	case TYPE_ENCODING:
	...
	case TYPE_PORT:
	    ...
	    port = (unsigned short)optionValue;
	    break;
	case TYPE_READONLY:
	    if (Tcl_GetBooleanFromObj(interp, objv[i+1], &optionValue)
		!= TCL_OK) {
...
ConnectionColumnsMethod(
	...
	for (i = 0; i < fieldCount; ++i) {
		...
		Tcl_DictObjPut(NULL, attrs, literals[LIT_PRECISION],
			       Tcl_NewWideUIntObj(field->length));
	    } else if (field->charsetnr < cdata->nCollations) {
		Tcl_DictObjPut(NULL, attrs, literals[LIT_PRECISION],
		    Tcl_NewWideUIntObj(field->length
			/ (unsigned)cdata->collationSizes[field->charsetnr]));
	    }
    ...
    Tcl_Obj *const *objv)           /* Parameter vector */
{
	...
	Tcl_ObjectGetMetadata(thisObject, &connectionDataType);
				/* Instance data */
    unsigned nColumns;		/* Number of columns in the result set */
	...
	if (nColumns == 0) {
	    Tcl_SetObjResult
		(interp,
		 Tcl_NewWideUIntObj(mysql_affected_rows(cdata->mysqlPtr)));
	    return TCL_OK;
	} else {
	    TransferMysqlError(interp, cdata->mysqlPtr);
	    return TCL_ERROR;
	...
	for (i = 0; i < nColumns; ++i) {
	    if (rowPtr[i] != NULL) {
		fieldObj = Tcl_NewStringObj(rowPtr[i], (Tcl_Size)lengths[i]);
	    } else {
...
AllocAndPrepareStatement(
	...
	if (mysql_stmt_prepare(stmtPtr, nativeSqlStr, (unsigned long)nativeSqlLen)) {
    ...
    Tcl_Obj *const *objv)	/* Parameter vector */
{
    ...
    Tcl_Size nParams;		/* Number of parameters of the statement */
    Tcl_Size i;
    ...
    sdata->columnNames = ResultDescToTcl(sdata->metadataPtr);
    Tcl_IncrRefCount(sdata->columnNames);

    Tcl_ListObjLength(NULL, sdata->subVars, &nParams);
    sdata->params = (ParamData*) ckalloc((size_t)nParams * sizeof(ParamData));
    for (i = 0; i < nParams; ++i) {
...
ResultSetConstructor(
    ...
    Tcl_Size nParams;		/* The parameter count on the statement */
    Tcl_Size nBound;		/* Number of parameters bound so far */
    ...
    Tcl_Size len;		/* Length of a bound parameter */
    ...
    rdata->rowCount = 0;
    rdata->resultErrors = (my_bool*) ckalloc((size_t)nColumns * sizeof(my_bool));
    rdata->resultNulls = (my_bool*) ckalloc((size_t)nColumns * sizeof(my_bool));
    resultLengths = rdata->resultLengths = (unsigned long*)
	ckalloc((size_t)nColumns * sizeof(unsigned long));
    rdata->resultBindings = resultBindings = MysqlBindAlloc((size_t)nColumns);
    ...
    rdata->paramBindings = MysqlBindAlloc((size_t)nParams);
    rdata->paramLengths = (unsigned long*) ckalloc((size_t)nParams
	...
	if (paramValObj != NULL) {
		...
		bufPtr = (char *)MysqlBindAllocBuffer(rdata->paramBindings, nBound,
					      (unsigned long)(len+1));
		memcpy(bufPtr, paramValStr, (size_t)len);
		rdata->paramLengths[nBound] = (unsigned long)len;
		MysqlBindSetLength(rdata->paramBindings, nBound,
				   &(rdata->paramLengths[nBound]));
		break;
...
static const char version[] = PACKAGE_VERSION "+" STRINGIFY(TDBC_MYSQL_VERSION_UUID)
...
Tdbcmysql_Init(
	...
	if (info.isNativeObjectProc == 2) {
	    Tcl_CreateObjCommand2(interp, "::tdbc::mysql::build-info",
	...
	Tcl_IncrRefCount(nameObj);
	Tcl_NewMethod2(interp, curClass, nameObj, 1, ResultSetMethods[i], NULL);
	Tcl_DecrRefCount(nameObj);
    }
	...
	mysqlClientLibName = Tcl_GetObjResult(interp);
	Tcl_IncrRefCount(mysqlClientLibName);

	if (strstr(Tcl_GetString(mysqlClientLibName), "maria") != NULL
	    || mysqlClientVersion >= 50100) {
	    mysqlClientAtLeast51 = 1;
	} else {
	    mysqlClientAtLeast51 = 0;
	}
    }
    ++mysqlRefCount;
    Tcl_MutexUnlock(&mysqlMutex);
...
DeletePerInterpData(
	...
	if (mysqlClientLibName != NULL) {
	    Tcl_DecrRefCount(mysqlClientLibName);
	    mysqlClientLibName = NULL;
	}
    }
    Tcl_MutexUnlock(&mysqlMutex);
}
